...
func TestAWS_authorizeToken(t *testing.T) {
	...
	tests := map[string]func(*testing.T) test{
		...
		"fail/cannot-validate-sig": func(t *testing.T) test {
			p, srv, err := generateAWSWithServer()
			assert.FatalError(t, err)
			defer srv.Close()
...
func TestAWS_AuthorizeSSHRevoke(t *testing.T) {
	type test struct {
		p     *AWS
		token string
		code  int
...
func TestAWS_AuthorizeSSHRekey(t *testing.T) {
	type test struct {
		p     *AWS
		token string
		code  int
...
func TestAWS_AuthorizeSSHRenew(t *testing.T) {
	type test struct {
		p     *AWS
		token string
		code  int

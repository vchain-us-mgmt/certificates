...
func (p *AWS) AuthorizeRenew(ctx context.Context, cert *x509.Certificate) error {
	if p.claimer.IsDisableRenewal() {
		return errs.Unauthorized(errors.Errorf("authorizeRenew: renew is disabled for aws provisioner %s", p.GetID()))
	}
	return nil
}
...
func (p *AWS) authorizeToken(token string) (*awsPayload, error) {
	...
	if err != nil {
		return nil, errs.Wrapf(http.StatusUnauthorized, err, "authorizeToken: error parsing aws token")
	}
	if len(jwt.Headers) == 0 {
		return nil, errs.InternalServerError(errors.New("error parsing token: header is missing"))
	}

	var unsafeClaims awsPayload
	if err := jwt.UnsafeClaimsWithoutVerification(&unsafeClaims); err != nil {
		return nil, errs.Wrap(http.StatusUnauthorized, err, "authorizeToken: error unmarshaling claims")
	}

	var payload awsPayload
	if err := jwt.Claims(unsafeClaims.Amazon.Signature, &payload); err != nil {
		return nil, errs.Wrap(http.StatusUnauthorized, err, "authorizeToken: error verifying claims")
	}

	// Validate identity document signature
	if err := p.checkSignature(payload.Amazon.Document, payload.Amazon.Signature); err != nil {
		return nil, errs.Wrap(http.StatusUnauthorized, err, "authorizeToken: invalid aws token signature")
	}

	var doc awsInstanceIdentityDocument
	if err := json.Unmarshal(payload.Amazon.Document, &doc); err != nil {
		return nil, errs.Wrap(http.StatusUnauthorized, err, "authorizeToken: error unmarshaling aws identity document")
	}

	switch {
	case doc.AccountID == "":
		return nil, errs.Unauthorized(errors.New("authorizeToken: aws identity document accountId cannot be empty"))
	case doc.InstanceID == "":
		return nil, errs.Unauthorized(errors.New("authorizeToken: aws identity document instanceId cannot be empty"))
	case doc.PrivateIP == "":
		return nil, errs.Unauthorized(errors.New("authorizeToken: aws identity document privateIp cannot be empty"))
	case doc.Region == "":
		return nil, errs.Unauthorized(errors.New("authorizeToken: aws identity document region cannot be empty"))
	}
	...
	if err = payload.ValidateWithLeeway(jose.Expected{
		...
		return nil, errs.Wrapf(http.StatusUnauthorized, err, "authorizeToken: invalid aws token")
	}

	// validate audiences with the defaults
	if !matchesAudience(payload.Audience, p.audiences.Sign) {
		return nil, errs.Unauthorized(errors.New("authorizeToken: invalid token - invalid audience claim (aud)"))
	}
	...
	if p.DisableCustomSANs {
		if payload.Subject != doc.InstanceID &&
			...
			return nil, errs.Unauthorized(errors.New("authorizeToken: invalid token - invalid subject claim (sub)"))
		}
	}
	...
	if len(p.Accounts) > 0 {
		...
		if !found {
			return nil, errs.Unauthorized(errors.New("authorizeToken: invalid aws identity document - accountId is not valid"))
		}
	}
	...
	if d := p.InstanceAge.Value(); d > 0 {
		if now.Sub(doc.PendingTime) > d {
			return nil, errs.Unauthorized(errors.New("authorizeToken: aws identity document pendingTime is too old"))
		}
	}
...
func (p *AWS) AuthorizeSSHSign(ctx context.Context, token string) ([]SignOption, error) {
	if !p.claimer.IsSSHCAEnabled() {
		return nil, errs.Unauthorized(errors.Errorf("authorizeSSHSign: ssh ca is disabled for aws provisioner %s", p.GetID()))
	}
